...
define(function (require, exports, module) {
    ...
    module.exports = {
        ...
        "test tab detection": function testTabDetection(next) {
            var s = new EditSession(["define({", "\tfoo:1,", "\tbar:2,", "\tbaz:{,", "\t\tx:3", "\t}", "})"]);
        ...
        "test empty session": function testEmptySession(next) {
        ...
        "!test one line": function testOneLine(next) {
            var s = new EditSession(["define({", "    foo:1,", "})"]);
            var indent = whitespace.$detectIndentation(s.doc.$lines);
            assert.equal(indent.ch, " ");
            assert.equal(indent.length, 4);
        ...
        "test 1 width indents": function test1WidthIndents(next) {
            var s = new EditSession(["define({", "    foo:1,", "})", "define({", "    bar:1,", "})", "     t", "      t", "     t", "      t", "     t", "      t", "     t", "      t"]);
            var indent = whitespace.$detectIndentation(s.doc.$lines);
            // assert.equal(indent.ch, " ");
            // assert.equal(indent.length, 4);
...
//# sourceMappingURL=whitespace_test-compiled.js.map