...
function jsFileList(path, filter) {
    ...
    return fs.readdirSync(path).map(function (x) {
        if (x.slice(-3) == ".js" && !filter.test(x) && !/\s/.test(x)) return x.slice(0, -3);
    }).filter(Boolean);
}
...
function checkModes() {
    modeList().forEach(function (modeName, i) {
        console.log(padNumber(i + 1, 3) + ") check: \x1B[33m" + modeName + "\x1B[0m");
        try {
        ...
        var m = new Mode();
        if (!("lineCommentStart" in m) && !("blockComment" in m)) console.warn("missing comment in " + modeName);
        if (!m.$id) console.warn("missing id in " + modeName);
    ...
    function testBlockComment(tokenizer, blockComment, modeName) {
        if (blockComment.lineStartOnly) return; // TODO test 
        var str = blockComment.start + " " + blockComment.end;
        str = blockComment.start + str;
        if (blockComment.nestable) str += blockComment.end;
        var data = tokenizer.getLineTokens(str, "start");
        var isBroken = data.tokens.some(function (t) {
            return !/comment/.test(t.type);
        });
        if (isBroken) console.warn("broken blockComment in " + modeName, data);
        if (!/start/.test(data.state)) console.warn("broken state after blockComment in " + modeName, data);
    }

    function testLineComment(tokenizer, commentStart, modeName) {
        var tokens = tokenizer.getLineTokens(commentStart + " ", "start").tokens;
        if (!/comment/.test(tokens[0].type)) console.warn("broken lineCommentStart in " + modeName);
    }
}
...
function generateTestData(names, force) {
    ...
    docs.forEach(function (docName) {
        ...
        if (!p[1]) return;
        var modeName;
        if (modes.indexOf(p[0]) != -1) modeName = p[0];else if (modes.indexOf(p[1]) != -1) modeName = p[1];else modeName = { "txt": "text", cpp: "c_cpp" }[p[1]];
...
function arrayDiff(a1, a2) {
    ...
    for (var i = 0; i < l; i++) {
        out.push("\n", padNumber(i + 1, 3), ") ");
        if (a1[i] !== a2[i]) out.push("\x1B[31m", a1[i], "\x1B[0m != \x1B[32m", a2[i], "\x1B[0m");else out.push(a1[i]);
    }
...
if (!arg) {
    test();
    checkModes();
} else if (/--?g(en)?/.test(arg)) generateTestData(process.argv.splice(3));else if (/--?c(heck)?/.test(arg)) checkModes(process.argv.splice(3));else if (/\d+/.test(arg)) test(parseInt(process.argv[2], 10) || 0);else testMode(arg, -1);

//# sourceMappingURL=highlight_rules_test-compiled.js.map