...
std::string FileInfo::date(dateFormat df) const
{
  ...
  dateStr = intToString(1 + pTm->tm_mon) + '/' + intToString(pTm->tm_mday) + '/' + intToString(1900 + pTm->tm_year);
  timeStr = intToString(pTm->tm_hour) + ':' + intToString(pTm->tm_min) + ':' + intToString(pTm->tm_sec);
  if (df == dateformat)
    return dateStr;
  if (df == timeformat)
    return timeStr;
...
int main(int argc, char *argv[])
{
  ...
  // std::string srcFile = Path::getFullFileSpec("FileSystem.o");
